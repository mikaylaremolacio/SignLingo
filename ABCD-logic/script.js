...
let detectionWindow = 2000;   // ms of detection duration, can be changed externally
let countdownTime = 5;        // seconds, can be changed externally
...
function startDetection() {
  ...
  countdownBox.textContent=`Get ready: ${countdownTime}`;
  ...
  const countdownInterval = setInterval(()=>{
    t--;
    if(t>0){
      countdownBox.textContent=`Get ready: ${t}`;
    } else {
